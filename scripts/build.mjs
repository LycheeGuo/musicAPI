...
const splayerTemplate = await fs.readFile("src/plugin-template.js", "utf8");
const lxTemplate = await fs.readFile("src/lx-template.js", "utf8");
...
const splayerUpdateUrl = `https://raw.githubusercontent.com/${repo}/main/dist/splayer-source.js`;
const lxUpdateUrl = `https://raw.githubusercontent.com/${repo}/main/dist/lx-source.js`;
...
function render(template, values, label) {
  ...
  if (left?.length) throw new Error(`${label} 构建后仍存在未替换 token: ${[...new Set(left)].join(", ")}`);
  return out;
}

const splayerOut = render(splayerTemplate, {
  "__PLUGIN_VERSION__": version,
  "__UPDATE_URL__": splayerUpdateUrl,
  ...
  "__EMBEDDED_CONFIG__": embeddedConfig,
}, "SPlayer");

const lxOut = render(lxTemplate, {
  "__PLUGIN_VERSION__": version,
  "__LX_UPDATE_URL__": lxUpdateUrl,
  "__HOMEPAGE__": homepage,
  "__AUTHOR__": author,
  "__EMBEDDED_CONFIG__": embeddedConfig,
}, "LX Music");
...
await Promise.all([
  fs.writeFile("dist/splayer-source.js", splayerOut, "utf8"),
  fs.writeFile("dist/lx-source.js", lxOut, "utf8"),
]);

console.log(`Built dist/splayer-source.js version=${version} repo=${repo}`);
console.log(`Built dist/lx-source.js version=${version} repo=${repo}`);
